...
const (
	FileTransferService_DownloadFolder_FullMethodName = "/filetransfer.FileTransferService/DownloadFolder"
)
...
type FileTransferServiceClient interface {
	DownloadFolder(ctx context.Context, in *FolderRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[FileChunk], error)
}
...
func (c *fileTransferServiceClient) DownloadFolder(ctx context.Context, in *FolderRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[FileChunk], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &FileTransferService_ServiceDesc.Streams[0], FileTransferService_DownloadFolder_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[FolderRequest, FileChunk]{ClientStream: stream}
...
type FileTransferService_DownloadFolderClient = grpc.ServerStreamingClient[FileChunk]
...
type FileTransferServiceServer interface {
	DownloadFolder(*FolderRequest, grpc.ServerStreamingServer[FileChunk]) error
...
func (UnimplementedFileTransferServiceServer) DownloadFolder(*FolderRequest, grpc.ServerStreamingServer[FileChunk]) error {
...
func _FileTransferService_DownloadFolder_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(FolderRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(FileTransferServiceServer).DownloadFolder(m, &grpc.GenericServerStream[FolderRequest, FileChunk]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FileTransferService_DownloadFolderServer = grpc.ServerStreamingServer[FileChunk]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _FileTransferService_DownloadFolder_Handler,
			ServerStreams: true,
		},
	},
